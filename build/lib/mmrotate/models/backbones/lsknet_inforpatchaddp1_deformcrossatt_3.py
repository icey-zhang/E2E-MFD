...
class LSKNet_inforpatchaddp1_deformcrossatt3(BaseModule):
    def __init__(self, img_size=224, in_chans=3, embed_dims_main=[64, 128, 256, 512], embed_dims_aux=[64, 128, 256, 512],
        ...
        self.fusion = Inforpatch_addfusion(embed_dims_main[fusion_stage - 1], embed_dims_main[fusion_stage - 1],
                                            patch=8, scale=4)
        ...
        for i in range(num_stages):
            patch_embed_aux1 = OverlapPatchEmbed(img_size=img_size if i == 0 else img_size // (2 ** (i + 1)),
                                            patch_size=7 if i == 0 else 3,
                                            stride=4 if i == 0 else 2,
                                            in_chans=in_chans if i == 0 else embed_dims_aux[i - 1],
                                            embed_dim=embed_dims_aux[i], norm_cfg=norm_cfg)

            block_aux1 = nn.ModuleList([Block(
                dim=embed_dims_aux[i], mlp_ratio=mlp_ratios[i], drop=drop_rate, drop_path=dpr_aux[cur_aux1 + j],norm_cfg=norm_cfg)
                for j in range(depths_aux[i])])
            norm_aux1 = norm_layer(embed_dims_aux[i])
            cur_aux1 += depths_aux[i]

            setattr(self, f"patch_embed_aux1_{i + 1}", patch_embed_aux1)
            setattr(self, f"block_aux1_{i + 1}", block_aux1)
            setattr(self, f"norm_aux1_{i + 1}", norm_aux1)

            patch_embed_aux2 = OverlapPatchEmbed(img_size=img_size if i == 0 else img_size // (2 ** (i + 1)),
                                            patch_size=7 if i == 0 else 3,
                                            stride=4 if i == 0 else 2,
                                            in_chans=in_chans if i == 0 else embed_dims_aux[i - 1],
                                            embed_dim=embed_dims_aux[i], norm_cfg=norm_cfg)

            block_aux2 = nn.ModuleList([Block(
                dim=embed_dims_aux[i], mlp_ratio=mlp_ratios[i], drop=drop_rate, drop_path=dpr_aux[cur_aux2 + j],norm_cfg=norm_cfg)
                for j in range(depths_aux[i])])
            norm_aux2 = norm_layer(embed_dims_aux[i])
            cur_aux2 += depths_aux[i]

            setattr(self, f"patch_embed_aux2_{i + 1}", patch_embed_aux2)
            setattr(self, f"block_aux2_{i + 1}", block_aux2)
            setattr(self, f"norm_aux2_{i + 1}", norm_aux2)
    ...
    def init_weights(self):
        ...
        if self.init_cfg is None:
            ...
            super(LSKNet_inforpatchaddp1_deformcrossatt3, self).init_weights()
    
    def freeze_patch_emb(self):
        self.patch_embed_main1.requires_grad = False
        self.patch_embed_aux1_1.requires_grad = False
        self.patch_embed_aux2_1.requires_grad = False

    @torch.jit.ignore
    def no_weight_decay(self):
        return {'pos_embed_main1', 'pos_embed_main2', 'pos_embed_main3', 'pos_embed_main4', 
                'pos_embed_aux1_1', 'pos_embed_aux1_2', 'pos_embed_aux1_3', 'pos_embed_aux1_4', 
                'pos_embed_aux2_1', 'pos_embed_aux2_2', 'pos_embed_aux2_3', 'pos_embed_aux2_4', 
    ...
    def forward_features(self, x):
        ...
        for i in range(self.fusion_stage):
            patch_embed_aux1 = getattr(self, f"patch_embed_aux1_{i + 1}")
            block_aux1 = getattr(self, f"block_aux1_{i + 1}")
            norm_aux1 = getattr(self, f"norm_aux1_{i + 1}")
            x_aux1, H, W = patch_embed_aux1(x_aux1)
            for blk in block_aux1:
                x_aux1 = blk(x_aux1)
            x_aux1 = x_aux1.flatten(2).transpose(1, 2)
            x_aux1 = norm_aux1(x_aux1)
            x_aux1 = x_aux1.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()

            patch_embed_aux2 = getattr(self, f"patch_embed_aux2_{i + 1}")
            block_aux2 = getattr(self, f"block_aux2_{i + 1}")
            norm_aux2 = getattr(self, f"norm_aux2_{i + 1}")
            x_aux2, H, W = patch_embed_aux2(x_aux2)
            for blk in block_aux2:
                x_aux2 = blk(x_aux2)
            x_aux2 = x_aux2.flatten(2).transpose(1, 2)
            x_aux2 = norm_aux2(x_aux2)
            x_aux2 = x_aux2.reshape(B, H, W, -1).permute(0, 3, 1, 2).contiguous()

        outs.append(self.fusion((x_aux1, x_aux2)))
        ...
        for i in range(self.fusion_stage, self.num_stages):
            ...
            outs.append(x_main + dckdainter((x_aux1, x_main)) + dckdainter((x_aux2, x_main)))
